...
- `app.py`: Main entry point. Defines the Flask application and its routes.
- `database/`: Contains database utility functions. `db.py` is the intended location for SQLite connection and schema management.
- `static/`: Public assets (CSS in `static/css`, JS in `static/js`).
